...
<x-app-layout>
    <div x-data="{
        ...
        validateAndConfirm() {
            ...
            const price = document.getElementById('price').value;
            const duration = document.getElementById('duration_minutes').value;
            const maxPax = document.getElementById('max_pax').value; // TAMBAHAN
            const extraPrice = document.getElementById('extra_price_per_pax').value; // TAMBAHAN
            const benefit = document.getElementById('benefit').value.trim();
            ...
            if (!price) return this.showError('Harga Paket wajib diisi.');
            if (!duration) return this.showError('Durasi Paket wajib diisi.');
            if (!maxPax) return this.showError('Batas Maksimal Orang (Max Pax) wajib diisi.'); // TAMBAHAN
            if (!extraPrice) return this.showError('Harga Tambahan per Orang wajib diisi (isi 0 jika gratis).'); // TAMBAHAN
            if (!benefit) return this.showError('List Benefit wajib diisi.');
            ...
            <div class="p-8 bg-white rounded-2xl border border-gray-200 shadow-sm">
                ...
                <form x-ref="editForm" action="{{ route('admin.packages.update', $package->id) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        {{-- INPUT BARU: MAX PAX --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Max orang <span
                                    class="text-red-500">*</span></label>
                            <input type="number" name="max_pax" id="max_pax"
                                value="{{ old('max_pax', $package->max_pax ?? 1) }}" required min="1"
                                class="w-full rounded-xl border-gray-300 focus:border-red-500 focus:ring-red-500 shadow-sm transition">
                            @error('max_pax')
                        ...
                        <div>
                            ...
                            <input type="number" name="extra_price_per_pax" id="extra_price_per_pax"
                                value="{{ old('extra_price_per_pax', $package->extra_price_per_pax ?? 0) }}" required
                                min="0"
                                class="w-full rounded-xl border-gray-300 focus:border-red-500 focus:ring-red-500 shadow-sm transition"
