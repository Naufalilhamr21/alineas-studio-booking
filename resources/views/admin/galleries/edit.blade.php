<x-app-layout>
    <div x-data="{
        // --- State Image Upload (Nilai Awal dari Database) ---
        photoName: null,
        photoPreview: '{{ asset('storage/' . $gallery->image_path) }}',
        ...
        handleFileUpload(event) {
            ...
            if (file.size > 2 * 1024 * 1024) {
                ...
                this.photoPreview = '{{ asset('storage/' . $gallery->image_path) }}';
                return;
            }
        ...
        // 2. Buka Konfirmasi (Tidak ada input wajib untuk text)
        validateAndConfirm() {
            this.modalType = 'confirm';
            this.modalTitle = 'Simpan Perubahan?';
            this.modalMessage = 'Data galeri foto akan diperbarui.';
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 min-h-[85vh] p-6 lg:p-8">
            ...
            <div
                ...
                <a href="{{ route('admin.galleries.index') }}"
                    class="bg-gray-700 text-white w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-900 transition shadow-sm">
                    &larr;
                </a>
                <h1 class="text-lg md:text-2xl font-extrabold text-gray-900 tracking-tight">
                    Edit <span class="text-red-600">Foto</span>
                </h1>
            </div>
            ...
            <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
                <form x-ref="editGalleryForm" action="{{ route('admin.galleries.update', $gallery->id) }}"
                    ...
                    <div class="pb-5">
                        ...
                        <div class="mt-2" x-show="!photoPreview" style="display: none;">
                            <div x-on:click.prevent="$refs.photo.click()"
                                class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-blue-50 hover:border-blue-400 transition">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6 text-center px-4">
                                    ...
                                    <p class="mb-2 text-sm text-gray-500 font-semibold">Klik untuk ganti foto</p>
                        ...
                        <select name="package_id" id="package_id"
                            ...
                            @foreach ($packages as $package)
                                <option value="{{ $package->id }}"
                                    {{ old('package_id', $gallery->package_id) == $package->id ? 'selected' : '' }}>
                                    {{ $package->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center p-4 border border-gray-200 rounded-xl bg-gray-50 hover:bg-blue-50 hover:border-blue-200 transition cursor-pointer"
                    ...
                    <div class="flex justify-end mt-6">
                        <button type="button" @click="validateAndConfirm()"
                            class="px-8 py-3 text-sm lg:text-md bg-red-600 hover:bg-red-700 text-white font-bold rounded-full shadow-sm">
                            Update Perubahan
                        </button>
                    </div>
                </form>
