                ...
                <div class="w-full md:flex-1">
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 min-h-[450px] relative">
                        ...
                        <div
                            ...
                            <div x-show="formattedDate" class="w-full sm:w-auto text-center sm:text-right mt-1 sm:mt-0">
                    ...
                    <div class="w-full md:flex-1 md:mt-[52px]"> {{-- Menyamakan tinggi dengan tombol kembali di kiri --}}
                        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 min-h-[450px] relative">
                            <div
                                ...
                                <div class="text-center sm:text-left">
                                    <h3 class="text-md lg:text-xl font-bold text-gray-800">Pilih Jam Foto</h3>
                                    <p class="text-sm text-gray-400 mt-1">Total Orang: <span
                                            class="font-bold text-gray-700" x-text="totalPax + ' Orang'"></span></p>
                                </div>
                                    ...
                                    <div class="pt-6 border-t border-gray-100">
                                        <form x-ref="bookingForm" action="{{ route('booking.store') }}"
                                            ...
                                            <button type="submit" :disabled="!selectedTime"
                                                ...
                                                class="w-full py-4 rounded-xl font-bold text-base lg:text-lg transition-all duration-300 flex justify-between px-6 items-center">
                                                <span>Lanjut Pembayaran</span>
                                                <span x-show="selectedTime" x-text="formatRupiah(totalPrice)"
                                                    class="bg-white/20 px-3 py-1 rounded-lg text-sm"></span>
                                            </button>
                                        </form>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('bookingSystem', (packageId, basePrice, maxPax, extraPrice) => ({
                ...
                init() {
                    ...
                    // --- PERBAIKAN VITE/PUSHER LOAD RACE CONDITION ---
                    // Menggunakan setTimeout agar Alpine memberi waktu bagi echo.js (Vite)
                    // untuk memuat object window.Echo sebelum memanggilnya.
                    setTimeout(() => {
                        if (typeof window.Echo !== 'undefined') {
                            console.log('Echo berhasil dimuat, mendengarkan kalender...');
                            window.Echo.channel('alineas.calendar')
                                .listen('.SlotLocked', (e) => {
                                    this.addLock(e.date, e.time, e.userId);
                                })
                                .listen('.SlotUnlocked', (e) => {
                            ...
                            console.error(
                                'Koneksi Real-time gagal: window.Echo tidak ditemukan.');
                        }
